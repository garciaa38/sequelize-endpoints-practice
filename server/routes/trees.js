...
router.post('/', async (req, res, next) => {
    try {
        ...
        const newTree = Tree.build({ tree: name,
                                      location: location,
                                      heightFt: height,
...
router.delete('/:id', async (req, res, next) => {
    try {
        const deleteTree = await Tree.findOne({ where: { id: req.params.id } });
        const doesTreeExist = await Tree.findAll({ attributes: ['id']});
        const treeId = Number(req.params.id);

        for (let i = 0; i < doesTreeExist.length; i++) {
            if (treeId === doesTreeExist[i].dataValues.id) {
...
router.put('/:id', async (req, res, next) => {
    try {
        ...
        const treeId = Number(req.params.id);
        if (id === treeId) {
            const updateTree = await Tree.findOne({ where: { id: req.params.id } });
            const doesTreeExist = await Tree.findAll({ attributes: ['id']});
